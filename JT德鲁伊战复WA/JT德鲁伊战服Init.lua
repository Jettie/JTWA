...
aura_env.OnShow = function()
    ...
    if IsInGroup() then
        ...
        if aura_env.config.whisperTarget then
            SendChatMessage("已对你使用 "..select(2, GetSpellInfo(spellId)).." ! 别睡了, 起来嗨!", "WHISPER", nil, destName)
        end
        ...
        if aura_env.config.reportInRaid then
            SendChatMessage("已对 "..destName.." 使用了 "..select(2, GetSpellInfo(spellId)).." ! 快叫他起床!", "RAID")
        end
