...
local thisSpellId = 54646
local thisSpellBuffId = 54648

local focusMagicReminderName = "JT_FOCUS_MAGIC_REMINDER"
local isVoice = function()
    return ( _G[focusMagicReminderName] == nil ) and aura_env.config.isVoice or false
end

--author and header
local AURA_ICON = 135754
...
local THIS_GLYPH_ID = nil
local BUFF_DURATION = 1800
local BUFF_DURATION_WITH_GLYPH = 1800

local buffIsAboutToExpireSoundFile = "Mage\\补专注.ogg"
local buffIsAboutToExpireTTSText = "快补道标"
local buffIsAboutToExpireTTSSpeed = 2
local buffIsAboutToExpireTimer
local alertTimeBeforeBuffExpire = 300

local buffRemovedSoundFile = "Mage\\专注断了.ogg"
...
aura_env.OnTrigger = function(event, ...)
    if event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        elseif subevent == "SPELL_AURA_REMOVED" and spellId == thisSpellBuffId and sourceGUID == myGUID then
            ...
            if now - lastCastSuccessTime < 0.5 then
                ...
                if now - lastCastSuccessTime < 0.2 or now - lastCastSuccessTime >= thisSpellDuration - 0.1 then
    ...
    elseif event == "GLYPH_UPDATED" then
        thisSpellDuration = BUFF_DURATION
    end
end
